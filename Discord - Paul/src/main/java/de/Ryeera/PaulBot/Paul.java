...
public class Paul extends ListenerAdapter {
    ...
    private static final File configFile = new File("paul.json");
    private static final File matchroomFile = new File("matchrooms.json");
    ...
    public static void main(String[] args) {
        try {
            ...
            String pathname = "logs" + File.separator + "Paul_" + Utils.formatTime(System.currentTimeMillis(), "yyyyMMdd_HHmmss") + ".log";
            logger = new DragoLogger(new File(pathname));
        } catch (Exception e) {
            e.printStackTrace();
            System.exit(1);
        }
        ...
        try {
            ...
            for (Guild guild : jda.getGuilds()) {
                guild.loadMembers()
                        .onSuccess(m -> logger.log("INFO", "Loaded " + m.size() + " members for " + guild.getName()));
            }
        ...
        if (message.startsWith(config.getString("prefix"))) {
            ...
            if (args[0].equalsIgnoreCase("matchroom") || args[0].equalsIgnoreCase("mr")) {
                ...
                if (args[1].equalsIgnoreCase("create") || args[1].equalsIgnoreCase("make")) {
                    ...
                    StringBuilder channelname = new StringBuilder(args[2]);
                    int nextid = 3;
                    ...
                    ChannelAction<Category> action = guild.createCategory(channelname.toString());
                    ...
                    final String cname = channelname.toString();
                    action.queue(c -> {
